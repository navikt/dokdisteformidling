...
import static no.nav.dokdisteformidling.TestingUtils.classpathToByteArray;
import static no.nav.dokdisteformidling.TestingUtils.classpathToString;
...
class Sdist001IT {
	...
	private void stubGetServiceRegistry() {
		stubFor(get(urlMatching("/serviceregistry/identifier/" + EformidlingConstants.TRYGDERETTEN_ORGNUMMER + "/process/" + AVTALTMELDING_PROCESS))
				.willReturn(aResponse()
						.withStatus(OK.value())
						.withHeader(CONTENT_TYPE, APPLICATION_JSON_VALUE)
