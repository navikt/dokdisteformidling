...
public class Qdist011Service {
    ...
    public SendDigitalPost distribuerForsendelseTilDPIService(DistribuerForsendelseTilDpi distribuerForsendelseTilDpi, Exchange exchange) {
        HentForsendelseResponseTo hentForsendelseResponseTo = administrerForsendelse.hentForsendelse(distribuerForsendelseTilDpi
                .getForsendelseId());
		final String konversasjonId = determineKonversasjonId(hentForsendelseResponseTo.getKonversasjonId());
        ...
        return bridgeMotSDPMapper.map(hentForsendelseResponseTo, konversasjonId, hentSikkerDigitalPostadresseResponseTo,
                dokumenttypeInfoTo, varselInfoTo, journalpostQdist011);
    }

    private String determineKonversasjonId(final String hentForsendelseKonversasjonId) {
        if (isBlank(hentForsendelseKonversasjonId)) {
            return UUID.randomUUID().toString();
        } else {
            return hentForsendelseKonversasjonId;
        }
    }
