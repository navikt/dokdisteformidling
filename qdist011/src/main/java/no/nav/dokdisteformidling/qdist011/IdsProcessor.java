...
import static no.nav.dokdisteformidling.qdist011.Qdist011Route.PROPERTY_BESTILLINGS_ID;
...
public class IdsProcessor implements Processor {
    ...
    private void setBestillingsIdAsPropertyAndAddCallIdToMdc(Exchange exchange) {
        ...
        if (callId == null) {
            throw new ForsendelseManglerPaakrevdHeaderFunctionalException("qdist011 har mottatt forsendelse uten påkrevd header callId");
        } else if (callId.trim().isEmpty()) {
            throw new ForsendelseManglerPaakrevdHeaderFunctionalException("qdist011 har mottatt forsendelse med tom header callId");
        }
    ...
    private void setForsendelseIdAsProperty(Exchange exchange) {
        ...
        if (forsendelseId == null) {
            throw new ForsendelseManglerForsendelseIdFunctionalException("qdist011 har mottatt forsendelse uten påkrevd forsendelseId");
        } else if (forsendelseId.trim().isEmpty()) {
            throw new ForsendelseManglerForsendelseIdFunctionalException("qdist011 har mottatt forsendelse med tom forsendelseId");
        }
